...
# 메인 윈도우
window = tkinter.Tk()
window.geometry("1000x400")  # 메인 윈도우 해상도 설정
window.title("FlowerFinder")
window.iconbitmap('{}\\icon\\flower.ico'.format(dir_path))
...
#폰트설정
Main_font = tkinter.font.Font(family="Courier New", size=10)

#캔버스,라벨 설정
canvas = tkinter.Canvas(window,width=400,height=300,background='white')
first_label = tkinter.Label(window,text='1st',font=Main_font)
second_label = tkinter.Label(window,text='2nd',font=Main_font)
third_label = tkinter.Label(window,text='3rd',font=Main_font)
...
label_Label=tkinter.Label(window,text=flower_text,justify='left',font=Main_font)

howtouse = "Open a flower image file,\nFlowerFinder will answer the species of that image.(16 species)"
htu_Label = tkinter.Label(window,text=howtouse,justify='left',font=Main_font)
...
button_Exit.place(x=950, y=10)
label_Label.place(x=830,y=50)
htu_Label.place(x=420,y=330)
